...
class PybulletRobotServer:
    def __init__(
        ...
        self.object_name_list = ['plastic_orange']
        self.urdf_object_list = []
        for object_name in self.object_name_list:
            object_loaded = self.pybullet_client.loadURDF(models_lib[object_name], [x, y, 0], quat, globalScaling=0.96, useFixedBase=False)
            self.urdf_object_list.append(object_loaded)
     

        #change the friction of the object
        for T_object in self.urdf_object_list:  
            self.pybullet_client.changeDynamics(T_object, -1, lateralFriction=1.1)
    ...
    def serve(self) -> None:
        ...
        while True:
            ...
            #print the transformation of the end effector in the object frame
            print(ee_transformation_object_frame)
