...
class PybulletRobotServer:
    def __init__(
        ...
        self.object_name_list = ['plastic_apple', 'plastic_banana',  'clear_box_1']
        self.splat_object_name_list = ['plastic_apple', 'plastic_banana',  'orange_bin']
        self.randomize_object_positions = [True, True, False]
        self.randomize_object_rotations = [False, True, True]
        self.rotation_values = [[0, 0], [-np.pi/2, np.pi/2], [-np.pi/6, np.pi/6]]
    ...
    def serve(self) -> None:
        ...
        while True:
            ...
            if skip_recording_first:
                ...
                #check y position of the objects
                if object_pos[1] > object_pos_2[1]:
                    object_order = [1, 0]
            ...
            for k in range(2):
                ...
                #go to first object
                self.follow_trajectory(all_paths[4*k + 0], 0, gripper_velocity=0.2, skip_recording_first=skip_recording_first)
    ...
    def follow_trajectory(self, path, gripper_pos, use_current_iters=True, gripper_velocity=2, skip_recording_first=0):
        k = 0
        loop_iters = 0
        current_iters = 0 
        while k < len(path):
